...
namespace scgFullBodyController
{
    public class GunManager : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            weaponUnlocked[0] = true; // default pistol selalu aktif
        }
        ...
        void Update()
        {
            ...
            else if (Input.GetKeyDown(KeyCode.Alpha2) && index != 1 && weapons.Length > 1 && weaponUnlocked[1])
            {
                if (!weapons[index].GetComponent<GunController>().firing && !weapons[index].GetComponent<GunController>().swapping
                    && !weapons[index].GetComponent<GunController>().aiming && weapons[index].GetComponent<GunController>().aimFinished
                    && !weapons[index].GetComponent<GunController>().reloading && !weapons[index].GetComponent<GunController>().cycling)
            ...
            else if (Input.GetKeyDown(KeyCode.Alpha3) && index != 2 && weapons.Length > 2 && weaponUnlocked[2])
            {
                if (!weapons[index].GetComponent<GunController>().firing && !weapons[index].GetComponent<GunController>().swapping
                    && !weapons[index].GetComponent<GunController>().aiming && weapons[index].GetComponent<GunController>().aimFinished
                    && !weapons[index].GetComponent<GunController>().reloading && !weapons[index].GetComponent<GunController>().cycling)
        ...
        public void UnlockWeapon(int weaponSlot)
        {
            if (weaponSlot < weaponUnlocked.Length)
            {
                weaponUnlocked[weaponSlot] = true;
            }
        }

        public bool IsWeaponUnlocked(int slot)
        {
            if (slot >= weaponUnlocked.Length) return false;
            return weaponUnlocked[slot];
        }
    }
}
